...
DATA_DIR = 'E:\Deep_learning_Final\dataset'
...
ANNOTATIONS_TRAIN = [os.path.join(DATA_DIR, 'annotations', item) for item in ['annotation_train.json']]
IMAGE_DIR_TRAIN = os.path.join(DATA_DIR, 'images/infant_train')
ANNOTATIONS_VAL = os.path.join(DATA_DIR, 'annotations', 'annotation_val.json')
IMAGE_DIR_VAL = os.path.join(DATA_DIR, 'images/infant_val')
...
def train_cli(parser):
    ...
    group.add_argument('--batch-size', default=6, type=int,
                       help='batch size')
    group.add_argument('--lr', '--learning-rate', default=1.0, type=float,
...
model.load_state_dict(torch.load('pose_model.pth'))   # load pre-trained model
...
print("The toal number of epochs: "+str(args.epochs))
...
for epoch in range(0, args.epochs):
    ...
    val_loss = validate(val_loader, model, epoch)
    print(val_loss)
    # train_loss_set.append(train_loss.to('cpu').numpy())   
...
df['epoch'] = epoch_set
# df['training_loss'] = train_loss_set
df['val_loss'] = val_loss_set 

df.to_csv('log.csv',columns=['epoch','val_loss'], header=True,index=False,encoding='utf-8')